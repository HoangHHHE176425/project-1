...
public class CartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (user == null) {
            ...
            if (service.equals("addToCart")) {
                ...
                //Gọi lại mehod để tính số lượng sản phẩm trong cart
                int totalNumberOfProductsInCart = calculateTotalNumberOfProductsInCart(session);
                session.setAttribute("numberProductsInCart", totalNumberOfProductsInCart);
            ...
            if (service.equals("update")) {
                ...
                while (em.hasMoreElements()) {
                    ...
                    if (!id.equals("user") && !id.equals("fullname") && !id.equals("numberProductsInCart")) {
                        ...
                        if (quantity <= 0) {
                            session.removeAttribute(id); // Xóa sản phẩm nếu số lượng <= 0
                        } else {
                ...
                // Gọi lại method để tính số lượng sản phẩm trong cart
                int totalNumberOfProductsInCart = calculateTotalNumberOfProductsInCart(session);
                session.setAttribute("numberProductsInCart", totalNumberOfProductsInCart);
    ...
    private int calculateTotalNumberOfProductsInCart(HttpSession session) {
        int totalQuantity = 0;
